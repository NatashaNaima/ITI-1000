    ...
    *  the index of the next move.
	...
	public TicTacToeGame(TicTacToeGame base, int next){
		...
		gameState = base.getGameState();
		 //copying elements of base game
		for(int i=0; i<base.lines*base.columns;i++){
			this.board[i]=(base.valueAt(i));
		}
		if(this.board[next] == CellValue.EMPTY){
			this.play(next);		
		}
    ...
    *  the TicTacToeGame instance to be compared with this one
	...
	public boolean equals(TicTacToeGame other) {
		...
		for(int i=0; i<size;i++){
			if(this.board[i] == CellValue.X && other.valueAt(i) != CellValue.X){
				return false;
			}
			else if(this.board[i] == CellValue.O && other.valueAt(i) != CellValue.O){
				return false;
			}
			else if(this.board[i] == CellValue.EMPTY && other.valueAt(i) != CellValue.EMPTY){
				return false;
			}
