...
public class TicTacToeGame {
	...
	*/
	...
	private Transformation[] square = new Transformation[]{Transformation.ID,Transformation.ROT,Transformation.ROT,
		Transformation.ROT,Transformation.HSYM,Transformation.ROT,Transformation.ROT,Transformation.ROT};
	private Transformation[] odd = new Transformation[]{Transformation.ID,Transformation.HSYM,Transformation.VSYM,Transformation.HSYM};
    ...
    *  the number of cells that must be aligned to win.
	...
	public TicTacToeGame(int lines, int columns, int sizeWin){
		...
		transCount = 0;
		if(lines == columns){
			transform = new int[8][lines*columns];
		}else{transform = new int[4][lines*columns];}
	}
    ...
    *  String representation of the game
    ...
    public void next(){
    	...
    	if(hasNext()==false){
    		throw new IllegalStateException("Exceeded possible symmetries");
    	}
    	...
    	if(trans[transCount] == Transformation.ID){
    		reset();
    	}
    	else if(trans[transCount] == Transformation.ROT){
    		Utils.rotate(lines,columns,transformedBoard);
    	}
    	else if(trans[transCount] == Transformation.HSYM){
    		Utils.horizontalFlip(lines,columns,transformedBoard);
    	}
    	else if(trans[transCount] == Transformation.VSYM){
    		Utils.verticalFlip(lines,columns,transformedBoard);
    	}
    	transform[transCount] = transformedBoard;
    	transCount++;
    }

    private boolean equals(TicTacToeGame other) {
  	...
  	*/    
  	public boolean equalsWithSymmetry(TicTacToeGame other){
    	...
    	for(int i=0;i<transform.length;i++){
    		CellValue[] thisBoard = new CellValue[lines*columns];
    		for(int j=0;j<lines*columns;j++){
    			thisBoard[j] = board[transform[i][j]];
    		}
    		if(thisBoard.equals(other)){
    			return true;
    		}
    	}
    	return false;
