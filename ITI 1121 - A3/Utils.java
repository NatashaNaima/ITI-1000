...
public class Utils {
    ...
    public static void rotate(int lines, int columns, int[] transformedBoard){
        ...
        int[] newboard = new int[lines*columns];
        int j = 0;

        //interating through board by columns
        for(int c=0;c<columns;c++){ 
            for(int i=lines-1; i>=0;i--){
                int index = columns*i + c;
                newboard[j] = transformedBoard[index];
                j++;
            }    
        }
        
        for(int i =0; i<lines*columns;i++){
            transformedBoard[i] = newboard[i];
        }  
    ...
    public static  void horizontalFlip(int lines, int columns, int[] transformedBoard){
        int[] newboard = new int[columns*lines];
        int j =0;
